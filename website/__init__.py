...
def initialize_app():
    ...
    class UserClass:
        ...
        def load_user(id):
            try:
                u = dbORM.find_one("UserFBET", "id", id)
                if not u:
                    return None
    ...
    def login():
        ...
        if request.method == 'POST':
            ...
            user = dbORM.find_one("UserFBET", "email", email)
    ...
    def register():
        if request.method == 'POST':
            ...
            if ref_code == "":
                ...
                try:
                    tUser = dbORM.get_all("UserFBET")[f'{dbORM.find_one("UserFBET", "username", ref_code)}']
                    dbORM.update_entry(
                        "UserFBET", 
                        f"{dbORM.find_one('UserFBET', 'id', str(tUser['id']))}", 
            ...
            if user:
                ...
                new_user = {
                    ...
                    "unique": f"{id_generator.generateTID()}",
                    "dob": dob,
                ...
                t_user = UserClass(id=f'{dbORM.find_one("UserFBET", "email", email)}')
